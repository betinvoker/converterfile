...
class DesktopApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Мое десктоп приложение")
        ...
        # Кнопка
        self.butLoadFile = ttk.Button(main_frame, text="Выберите файлы для конвертации", command=self.on_but_click_load_file)
       ...
       # Вторая строка: разделитель конвертируемого файла
        ...
        # Кнопка
        self.butRun = ttk.Button(main_frame, text="Выполнить конвертацию файлов", command=self.on_btn_run_convert)
    ...
    def on_btn_run_convert(self):
        ...
        for line in lines:
            if line.strip() and not line.startswith("Выбрано файлов:") and not line.startswith("Обработано файлов:"):
                ...
                if file_path and not file_path.startswith("Файлы не выбраны") and not file_path.startswith("Файл ") and not line.startswith("  Путь:") and not line.startswith("  Имя:") and not line.startswith("  Формат:") and not line.startswith("  Разделитель") and not line.startswith("  Целевой формат:") and not line.startswith("-" * 40):
        ...
        try:
            ...
            if conv_file.format == 'xlsx':
                ...
                if target_format == 'txt':
                    output_filename = f"{conv_file.name}_converted.txt"
                    df.to_csv(output_filename, sep=delimiter_convert, index=False)
                elif target_format == 'csv':
                    output_filename = f"{conv_file.name}_converted.csv"
                    df.to_csv(output_filename, sep=delimiter_convert, index=False)
                ...
                return f"Неизвестный формат файла: {conv_file.format}"
